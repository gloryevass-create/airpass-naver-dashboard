...
export async function ensureFileShared(
  fileId: string
): Promise<{ name: string; link: string; mimeType: string; iconLink: string | null }> {
  const drive = getDriveClient();
  ...
  const file = await drive.files.get({ fileId, fields: "name, webViewLink, mimeType, iconLink" });
  ...
  return {
    ...
    iconLink: file.data.iconLink ?? null,
  };
}
