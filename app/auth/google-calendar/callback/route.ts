...
export async function GET(request: NextRequest) {
  ...
  if (oauthError || !code || !state || state !== savedState) {
    return NextResponse.redirect(`${origin}/dashboard/calendar?googleError=1`);
  }
  ...
  try {
    ...
    if (!tokens.refresh_token) {
      ...
      return NextResponse.redirect(`${origin}/dashboard/calendar?googleError=no_refresh_token`);
    }
  ...
  return NextResponse.redirect(`${origin}/dashboard/calendar?googleConnected=1`);
}
